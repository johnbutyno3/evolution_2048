...
class _ShopPageState extends State<ShopPage> {
  ...
  void _openTool(GameToolType type) {
    Navigator.of(context).push(
      MaterialPageRoute(builder: (_) => ToolPurchasePage(tool: type)),
    );
  }
        ...
        builder: (context, _) {
          return ListView(
            ...
            children: [
              Card(
                child: ListTile(
                  leading: const Icon(Icons.monetization_on_outlined, size: 34),
                  title: const Text('金幣'),
                  subtitle: Text('累計消費 ${GoldManager.lifetimeSpent} 金幣'),
                  trailing: Text(
                    '${GoldManager.balance}',
              ...
              const Text('金幣儲值', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              const _GoldPackageTile(gold: 500, price: 'NT\$49'),
              const _GoldPackageTile(gold: 1000, price: 'NT\$89'),
              const _GoldPackageTile(gold: 5000, price: 'NT\$399'),
              const _GoldPackageTile(gold: 10000, price: 'NT\$799'),
              const SizedBox(height: 20),
              const Text('道具', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              _ToolTile(type: GameToolType.timeRewind, onTap: () => _openTool(GameToolType.timeRewind)),
              _ToolTile(type: GameToolType.revive, onTap: () => _openTool(GameToolType.revive)),
              _ToolTile(type: GameToolType.positionSwap, onTap: () => _openTool(GameToolType.positionSwap)),
              _ToolTile(type: GameToolType.duplicate, onTap: () => _openTool(GameToolType.duplicate)),
              const SizedBox(height: 20),
              const Text('生命', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Card(
                child: ListTile(
                  leading: const Icon(Icons.favorite_outline),
                  title: const Text('生命 +1'),
                  subtitle: const Text('50 金幣'),
...
class ToolPurchasePage extends StatefulWidget {
  const ToolPurchasePage({super.key, required this.tool});
...
class _ToolPurchasePageState extends State<ToolPurchasePage> {
  ...
  Future<void> _buy() async {
    ...
    await ToolManager.addPurchasedUses(widget.tool, quantities[_selected]);
    _ShopRefreshNotifier.instance.refresh();
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('已購買 ${quantities[_selected]} 次 $_name')),
    );
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('購買 $_name')),
      body: ListView(
        ...
        children: [
          ...
          ...List.generate(quantities.length, (index) {
            ...
            return Card(
              child: RadioListTile<int>(
                ...
                subtitle: Text('${_priceFor(quantity)} 金幣'),
                selected: selected,
              ),
            );
          }),
          const SizedBox(height: 12),
          FilledButton.icon(
            onPressed: GoldManager.balance >= _price ? _buy : null,
            icon: const Icon(Icons.shopping_cart_outlined),
            label: Text('購買 $_price 金幣'),
          ),
        ],
      ),
    );
...
class _ToolTile extends StatelessWidget {
  const _ToolTile({required this.type, required this.onTap});
...
class _GoldPackageTile extends StatelessWidget {
  const _GoldPackageTile({required this.gold, required this.price});
  ...
  Widget build(BuildContext context) {
    return Card(
      child: ListTile(
        ...
        subtitle: const Text('付款功能將於正式商店串接後啟用'),
      ),
    );
  }
}
...
class _ShopRefreshNotifier extends ChangeNotifier {
  ...
  static final _ShopRefreshNotifier instance = _ShopRefreshNotifier._();
