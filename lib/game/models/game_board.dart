...
class GameBoard {
  ...
  bool moveRight() {
    return _move(
      (row, column) =>
          _index(row, _size - 1 - column),
    );
  }

  bool moveUp() {
    return _move(
      (column, row) => _index(row, column),
    );
  }
  ...
  bool _move(
    int Function(int line, int position) indexFor,
  ) {
    var changed = false;
    ...
    for (var line = 0; line < _size; line++) {
      ...
      for (var target = 0;
          target < _size;
          target++) {
        final tile = target < merged.length
            ? merged[target]
            : null;

        final index =
            indexFor(line, target);
